...
def test_loads_t3_with_real_geometry_trigger():
    scenario = load_scenario(3)

    assert scenario["fire_crosses_line_a"] is True
    assert scenario["trigger_source"] == "geometry_intersection"
    assert scenario["fire_line_distance_m"] == 0
